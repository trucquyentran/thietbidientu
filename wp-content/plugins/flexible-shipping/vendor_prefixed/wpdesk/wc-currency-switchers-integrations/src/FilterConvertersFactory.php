...
class FilterConvertersFactory implements \FSVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function hooks()
    {
        ...
        \add_action('woocommerce_init', array($this, 'create_currency_switcher_woocommerce_filter_converter'), self::PRIORITY_AFTER_DEFAULT);
        \add_action('wpml_loaded', array($this, 'create_wpml_filter_converter'));
        \add_action('woocommerce_init', array($this, 'create_aelia_filter_converter'), self::PRIORITY_AFTER_DEFAULT);
        \add_action('woocommerce_init', array($this, 'create_fox_currency_switcher_filter_converter'), self::PRIORITY_AFTER_DEFAULT);
        \add_action('woocommerce_init', array($this, 'create_wmcs_filter_converter'), self::PRIORITY_AFTER_DEFAULT);
    }
    ...
    public function create_wpml_filter_converter()
    {
        (new \FSVendor\WPDesk\WooCommerce\CurrencySwitchers\FilterConverter(new \FSVendor\WPDesk\WooCommerce\CurrencySwitchers\Switcher\WPML\Converter(), $this->shipping_method_id))->hooks();
    }
